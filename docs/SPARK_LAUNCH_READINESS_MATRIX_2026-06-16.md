...
| Natural preview readout authority | 2026-06-16 04:59 Telegram: "where can I open the latest preview?" produced a read-only Spawner board readout instead of silence or a build; it did not promote the accidental "what happened" mission | Live CUA pass after parser, natural route, Harness policy, and Spawner selector fixes |
| Spawner read-only route policy | `spark-telegram-bot` tests now prove latest-preview questions select `spawner.board/latest_project_preview`, allow only `spawner.board` read authority, and deny `spawner.run` | Unit authority pass for parser, route, Harness/Governor, and owner readout layers |
| Failure residue selector guard | `latestFailureSummary`, `latestFailedProviderSummary`, and `latestProjectPreview` skip operational probes and question/readout residue when promoting latest job truth | Unit pass for failed and completed residue cases |
| Builder fallback completion-claim gate | `spark-telegram-bot` now suppresses Builder/provider fallback and direct Builder voice/media/text replies that claim done/ready/fixed/shipped/open-preview without owner proof | Focused unit/E2E pass; live Telegram follow-up still pending |
| Live completion-wording no-action probe | 2026-06-16 05:13 Telegram: quoted "the build is done" as a wording risk with "Do not build or start anything"; Spark answered with owner-proof requirements and did not create a mission | Live CUA pass; route ledger matched `conversation.quoted_drafted_example_boundary` through answer boundary |
...
| --- | --- | --- | --- | --- | --- |
| Spawner continuum | Natural ideation, build start, PRD/canvas, Kanban, provider execution, preview, iteration, status/failure readout, rerun | Recent fixes cover mission failure/status, contextual project iteration, board/latest-preview/failure residue, and stale-provider readouts. Live CUA proved failure, `/board`, and natural preview readouts. | 8 | Yellow | Run broader live Telegram flow: vague idea -> advice -> explicit apply -> canvas -> completion/failure -> status/readout. |
| Telegram routing/authority | Natural chat, slash commands, "ok do it", no-action/meta talk, status questions, access questions, mission controls | Broad E2E suite and route matrix pass for touched routes. Harness policy now denies launch/file authority to Spawner board read routes. Live CUA used for mission status, `/board`, failure, and preview readout regressions. | 8 | Yellow | Run mixed prompt pack against Telegram Desktop and inspect route ledgers for mismatches. |
...
| Builder bridge | Plain chat, provider fallback, final-answer gate, unsupported edit/completion claims, memory-route boundary, direct voice/media delivery | E2E tests show unsupported edit and unproved completion claims are suppressed with trace ids; direct Builder voice/media delivery now inherits the same claim gate. Live Telegram CUA proved quoted completion wording stayed answer-only with no mission side effect. | 7 | Yellow | Test natural chat and actual Builder fallback failure modes via Telegram; verify no edit/done claims without owner proof. |
