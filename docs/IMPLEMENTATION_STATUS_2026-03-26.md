...
- operators can now explicitly snooze, inspect, and clear one webhook-alert family through `operator snooze-webhook-alert <event>`, `operator webhook-alert-snoozes`, and `operator clear-webhook-alert-snooze <event>`, while active snoozes remain visible in `operator inbox` and `operator security` with their operator reason and snoozed time when present, clear actions retain the prior snooze metadata in operator history, and expired snoozes self-prune from local runtime state
