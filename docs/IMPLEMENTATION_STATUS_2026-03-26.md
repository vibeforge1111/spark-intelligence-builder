...
- first Discord webhook skeleton on `/webhooks/discord` with route validation, Discord signature verification via interaction public key, compatibility fallback secret auth for the stub path, signed `PING` handling, and DM-only `APPLICATION_COMMAND` routing through the existing DM bridge
