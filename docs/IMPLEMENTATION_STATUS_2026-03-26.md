...
- first WhatsApp webhook skeleton on `/webhooks/whatsapp` with route validation, Meta-style GET verification, `X-Hub-Signature-256` POST auth, fail-closed JSON parsing, one explicit Meta text-message event contract routed through the existing simulated WhatsApp bridge instead of opportunistic stub or batch normalization, and trace-visible ignore reasons in `gateway traces`
