...
1. Move the loopback callback listener from CLI-owned flow to a fuller gateway-owned HTTP surface without weakening the current fail-closed posture.
2. Add operator-visible reconnect and revoke guidance on top of the new OAuth refresh and expiry states.
