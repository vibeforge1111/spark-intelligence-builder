...
- first WhatsApp webhook skeleton on `/webhooks/whatsapp` with route validation, Meta-style GET verification, `X-Hub-Signature-256` POST auth, fail-closed JSON parsing, and supported text-message webhook payloads routed through the existing simulated WhatsApp bridge
