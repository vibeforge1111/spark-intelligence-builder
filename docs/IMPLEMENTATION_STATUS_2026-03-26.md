...
- first Discord webhook skeleton on `/webhooks/discord` with route validation, Discord signature verification via interaction public key, compatibility fallback secret auth for the stub path, signed `PING` handling, and an explicit DM-only `/spark message:<text>` `APPLICATION_COMMAND` contract routed through the existing DM bridge
