...
The first WhatsApp webhook skeleton is now also in place on `/webhooks/whatsapp`. It now supports the Meta-style GET verification handshake with `hub.challenge`, verifies POST payload authenticity using `X-Hub-Signature-256`, fails closed on malformed JSON, and currently routes supported text-message webhook payloads through the existing simulated WhatsApp bridge.
