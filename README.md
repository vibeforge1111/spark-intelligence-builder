...
The first real webhook skeleton now uses that contract for Discord. `/webhooks/discord` is registered as POST `application/json`, request validation happens before JSON decoding, JSON decoding happens before Discord normalization, and a configured `X-Spark-Webhook-Secret` must match the Discord webhook secret stored in `.env`. That keeps the ingress boundary strict without pretending Discord signature or remote auth verification is finished yet.
