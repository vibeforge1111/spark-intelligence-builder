    ...
    def test_build_researcher_reply_archives_stale_structured_evidence_when_newer_evidence_exists(self) -> None:
        ...
        with patch("spark_intelligence.memory.orchestrator._now_iso", return_value="2025-02-01T00:00:00+00:00"):
            build_researcher_reply(
                config_manager=self.config_manager,
                state_db=self.state_db,
                request_id="req-open-evidence-old-write",
                agent_id="agent-1",
                human_id="human-1",
                session_id="session-open-evidence-old-write",
                channel_kind="telegram",
                user_message="Users keep dropping during onboarding because Stripe verification fails.",
            )
        ...
        with patch("spark_intelligence.memory.orchestrator._now_iso", return_value="2025-03-20T00:00:00+00:00"):
            build_researcher_reply(
                config_manager=self.config_manager,
                state_db=self.state_db,
                request_id="req-open-evidence-new-write",
                agent_id="agent-1",
                human_id="human-1",
                session_id="session-open-evidence-new-write",
                channel_kind="telegram",
                user_message="Users still drop during onboarding because Stripe verification fails and the identity retry flow is confusing.",
            )
        ...
        self.assertEqual(archive_observations[0].get("structured_evidence_lifecycle_action"), "archived")
