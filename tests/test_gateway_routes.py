...
class GatewayRouteRegistryTests(SparkTestCase):
    ...
    def test_root_prefix_route_remains_a_catch_all(self) -> None:
        registry = GatewayRouteRegistry()
        registry.register(
            GatewayRouteRegistration(
                path="/",
        ...
        self.assertEqual(registry.resolve(path="/health", method="GET").owner, "gateway-root")
        self.assertEqual(registry.allowed_methods_for_path(path="/health"), ("GET",))
