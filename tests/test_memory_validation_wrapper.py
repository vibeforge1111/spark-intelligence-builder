...
def test_validated_full_cycle_runs_preflight_before_full_validation_and_passes_skip_publish(tmp_path: Path) -> None:
    real_builder_root = Path(__file__).resolve().parents[1]
    ...
    trace_path = tmp_path / "validated-full-cycle-trace.jsonl"
...
$entry = [ordered]@{{ step = "preflight" }} | ConvertTo-Json -Compress
Add-Content -Path "{trace_path}" -Value $entry
""".strip(),
    )
    _write_text(
...
""".strip(),
    ...
    assert completed.returncode == 0, completed.stderr or completed.stdout
    assert "== Memory automation preflight ==" in completed.stdout
    assert "== Memory full validation ==" in completed.stdout
    ...
    assert full_entry["output_root"] == str(tmp_path / "output-root")
    assert full_entry["soak_runs"] == 3
    assert full_entry["soak_timeout_seconds"] == 123.0
    assert full_entry["skip_baseline_publish"] is True


def test_validated_full_cycle_auto_skips_baseline_publish_for_non_default_runs(tmp_path: Path) -> None:
    real_builder_root = Path(__file__).resolve().parents[1]
    ...
    trace_path = tmp_path / "validated-full-cycle-auto-skip-trace.jsonl"
...
""".strip(),
    ...
    assert completed.returncode == 0, completed.stderr or completed.stdout
    assert "Baseline publish mode: skipped" in completed.stdout
