...
class ContextCapsuleTests(SparkTestCase):
    ...
    def test_cleanup_closure_evidence_does_not_overclaim_sample_review(self) -> None:
        ...
        self.assertIn("  - Archived examples: profile.current_focus: diagnostics scan verification", result.reply_text)
        self.assertIn("  - Deleted examples: profile.current_owner: delete profile.current_owner", result.reply_text)
        self.assertIn("  - Still-current examples: profile.current_focus: context capsule verification", result.reply_text)
