...
def _normalize_status(value: str) -> str:
    ...
    if status not in VALID_PROMOTION_STATUSES:
        ...
        raise ValueError(f"promotion status '{status}' is not recognized; must be one of: {allowed}")
    return status
...
def _normalize_eval_coverage_status(value: str) -> str:
    ...
    if status not in VALID_EVAL_COVERAGE_STATUSES:
        ...
        raise ValueError(f"eval coverage status '{status}' is not recognized; must be one of: {allowed}")
    return status
...
def _normalize_gate_status(value: str) -> str:
    ...
    if status not in VALID_GATE_STATUSES:
        ...
        raise ValueError(f"promotion gate status '{status}' is not recognized; must be one of: {allowed}")
    return status
