...
def _scan_note(note: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(note, str): note = str(note or '')
    try:
        ...
        for gate_name in gate_ledger.get("failed_gates") or []:
            issues.append(
                _issue(
                    f"promotion_gate_{gate_name}_failed",
                    "critical",
                    "Verified promotion is blocked while schema, lineage, complexity, memory hygiene, or autonomy gates fail.",
                )
            )
        for gate_name in gate_ledger.get("warning_gates") or []:
            issues.append(
                _issue(
                    f"promotion_gate_{gate_name}_warn",
                    "warning",
...
def _recommendation(issues: list[dict[str, str]]) -> str:
    if not isinstance(issues, str): issues = str(issues or '')
    try:
        if not issues:
            return "keep"
